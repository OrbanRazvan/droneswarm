...
// VITEZA DRONEI PRINCIPALE IN PLAY VS AI.
// Schimba valoarea asta daca vrei mai incet/mai rapid.
// Tine aceeasi valoare ca GAME_FRAME_SPEED din PvpArena.jsx.
...
// Body collision / ram system.
// Nu mai impinge instant urat pe un singur frame.
// Damage-ul se aplica instant, dar separarea se face smooth prin knockbackVelocity,
// iar miscarea se consuma treptat in game loop.
...
function getProjectilePrimaryColor(skin = "cyan") {
  return (DRONE_SKIN_THEMES[skin] || DRONE_SKIN_THEMES.cyan)[0];
}

function getProjectileGlowColor(skin = "cyan") {
  return (DRONE_SKIN_THEMES[skin] || DRONE_SKIN_THEMES.cyan)[4];
}
...
// Zona incepe in functie de marimea hartii.
// La harta 16000x16000, cercul initial este ~7520px, deci aproape toata harta este safe.
...
function getNextDroneAt(currentDrones = 0) {
  // Gameplay mai rapid:
  // prima drona mica apare repede, apoi progresiv mai greu.
  // 0/5 -> 0/15 -> 0/25 -> 0/35
...
function applyKillRewardToUnit(unit) {
  ...
  return {
    ...
    // Recompensa de kill:
    // agresivitatea iti da imediat munitie inapoi.
    drones: nextDrones,
    progress: 0,
    nextDroneAt: getNextDroneAt(nextDrones),

    // Streak reward temporar:
    // 3+ kill-uri = attack mai rapid si proiectile putin mai rapide.
...
function getRandomSpawnPoint(existingPoints = [], minDistance = BOT_MIN_SPAWN_DISTANCE) {
  ...
  // Fallback: ramane tot in zona safe, dar cu distanta mai relaxata.
...
function getZoneEscapeMove(bot, radius) {
  const centerX = WORLD_WIDTH / 2;
  const centerY = WORLD_HEIGHT / 2;
  ...
  const side = bot.id.charCodeAt(0) % 2 === 0 ? 1 : -1;
  const tangentX = -toCenterY * side;
  const tangentY = toCenterX * side;

  const outsideAmount = Math.max(0, distance - radius);
  const centerForce = outsideAmount > 220 ? 4.4 : outsideAmount > 80 ? 3.6 : 2.8;

  return normalizeMove(
    toCenterX * centerForce + tangentX * 0.28,
    toCenterY * centerForce + tangentY * 0.28
  );
}
...
function getBotAvoidance(bot, bots) {
  let avoidX = 0;
  let avoidY = 0;
...
function canUnitUseCore(unit, core) {
  ...
  // Orice unit poate avea maxim 2 core-uri echipate/active TOTAL:
  // Nano, Rotor, Piercing, Overclock, Berserk, Shield Breaker, Swarm, Vampire, EMP.
...
function createBot(index, spawnPoint = null) {
  return {
    ...
    knockbackX: 0,
    knockbackY: 0,
    desiredDroneStock: 5 + Math.floor(Math.random() * 3),

    totalCollected: 0,
    progress: 0,
    nextDroneAt: getNextDroneAt(0),

    aiAggression: 0.75 + Math.random() * 0.5,
    aiCourage: 0.85 + Math.random() * 0.35,
    aiSkill: 0.8 + Math.random() * 0.4,
    preferredRange: 560 + Math.random() * 190,
...
function findBotEnemy(bot, player, bots) {
  ...
  enemies.forEach((enemy) => {
    ...
    const enemyWeak = enemy.hp <= 50 || enemy.drones <= 1;
    const hasDroneAdvantage = bot.drones >= enemy.drones + 2;
    const hasEnoughAmmo = bot.drones >= 2;
    const canWin =
      hasEnoughAmmo &&
      (hasDroneAdvantage || enemyWeak || botPower >= enemyPower * 1.05);

    // Nu urmari tinte daca nu ai drone mici. Botul trebuie sa farmeze.
    if (bot.drones <= 0) return;

    // Daca are putine drone, ataca doar tinte foarte slabe sau cu avantaj clar.
    if (bot.drones < BOT_FARM_UNTIL_DRONES && !enemyWeak && !hasDroneAdvantage) {
      return;
    }

    let score = distance;

    if (enemy.type === "player") score -= 60;
    if (enemyWeak) score -= 260;
    if (hasDroneAdvantage) score -= 320;
    if (canWin) score -= 180;
    if (enemy.hp < bot.hp) score -= 80;

    if (!canWin) score += 420;
    if (bot.hp <= BOT_LOW_HP && enemyPower > botPower) score += 650;
    if (distance < 260 && enemyPower > botPower) score += 380;
...
function GameArena({ user, gameMode = "ai", onExitToMenu }) {
  const keys = useRef({});
...
const leaderboardPlayers = useMemo(() => {
  ...
  const triggerPlayerDeath = (p, killerBotId = null) => {
    ...
    setMatchSummary(buildMatchSummary(winner || "Still playing"));
  ...
  const finishMatch = (matchWinner) => {
    ...
    setMatchSummary(buildMatchSummary(matchWinner));
    setGameOver(true);
  ...
  /*
    Mobile detection corect:
    - NU mai folosim width/height ca sa activam mobile mode.
    - Asa nu se mai activeaza modul mobile pe desktop cand imparti ecranul in doua.
    - Mobile mode se activeaza doar pe device real touch/mobile.
  */
  ...
  // Nu mai blocam Play vs AI pe telefon vertical.
  // Afisam joystick + Attack/Shield ca in PvpArena.
  const showRotateOverlay = false;

  const showMobileControls = player.alive && !gameOver && isMobileLike;

  const mobileWorldScale = 1;
  const mobileLootScale = isMobileLandscape ? 0.72 : 1;
  ...
  useEffect(() => {
    if (player.alive || gameMode !== "ai") return;
  ...
  }, [player.alive, gameMode, spectatorTargetId, bots]);
  ...
  const damagePlayer = (killerBotId = null, incomingDamage = HIT_DAMAGE, piercesShield = false, vampireOwnerType = null, vampireOwnerId = null) => {
    ...
    if (nextDrones > 0) {
      // Noua logica agresiva:
      // drona mica protejeaza partial, dar lovitura tot conteaza.
      // Targetul pierde 1 drona si 25 HP.
      ...
      // Daca nu are drone mici, incaseaza damage mare direct.
  ...
  const damageBot = (botId, x, y, killerType = "player", killerBotId = null, incomingDamage = HIT_DAMAGE, piercesShield = false, vampireOwnerType = null, vampireOwnerId = null) => {
    ...
    const updatedBots = botsRef.current.map((bot) => {
      ...
      if (nextDrones > 0) {
        // Noua logica:
        // botul pierde 1 drona, dar primeste si 25 HP damage.
        nextDrones = Math.max(0, nextDrones - 1);
        nextHp = Math.max(0, nextHp - DRONE_HIT_DAMAGE);
  ...
  const updateMobileAimFromPoint = (clientX, clientY, options = {}) => {
    ...
    // Daca degetul este prea aproape de centru, folosim directia de mers.
    if (distance < 18) {
    ...
    // IMPORTANT:
    // Nu mai setam player.attacking pe mobile in timp ce tii apasat.
    // In DroneSwarm, player.attacking muta orbita dronelor mici si de aici
    // aparea invartirea instabila/oribila. Actualizam doar directia de tragere.
    playerRef.current = {
      ...p,
  ...
  const updateMobileAimFromAttackButton = (e) => {
    ...
    // Ca in PvpArena: butonul ATTACK devine joystick de tinta.
    // Nu mai trebuie sa duci degetul pana la drona mare.
  ...
  const handleMobileAttackStart = (e) => {
    ...
    // Ramane false ca sa NU afecteze orbita dronelor mici.
    playerRef.current = {
      ...playerRef.current,
      attacking: false,
    };
  ...
  useEffect(() => {
    ...
    const loop = (now = performance.now()) => {
      ...
      if (gameMode === "ai" && Date.now() - lastBotEnergyDrainRef.current >= 1000) {
        lastBotEnergyDrainRef.current = Date.now();
      ...
      if (
        gameMode === "ai" &&
        nowBotLogic - lastBotLogicUpdateRef.current > botLogicInterval
      ) {
        ...
        const updatedBots = botsRef.current.map((bot) => {
          ...
          const avoidance = getBotAvoidance(bot, botsRef.current);
          ...
          // METODA NOUA: finite-state AI, nu schimbam tinta in fiecare frame.
          // Daca botul atinge zona verde, intra in escape lock si merge spre interior.
          // Nu farmeaza, nu ataca, nu face avoidance puternic pana nu e clar in siguranta.
          if (mustEscapeZone) {
          ...
          // Daca este aproape de marginea zonei, nu mai accepta orburi/tinte in directia aia.
          // Se reorienteaza spre interior si tine minte ca zona aceea e periculoasa.
          if (zoneInfo.dangerDistance < BOT_ZONE_EDGE_BUFFER) {
            const move = normalizeMove(
          ...
          const hasDroneAdvantage =
            enemyTarget && bot.drones >= enemyTarget.drones + 2;
          const enemyWeak =
            enemyTarget && (enemyTarget.hp <= 50 || enemyTarget.drones <= 1);
          const beingThreatened =
            enemyTarget &&
            enemyTarget.distance < 520 &&
            enemyPower > botPower * 0.92;

          const shouldKeepFarming =
            bot.drones < desiredStock &&
            !beingThreatened &&
            !(enemyWeak && bot.drones >= 2);

          const shouldFlee =
            enemyTarget &&
            (bot.hp <= BOT_LOW_HP || enemyPower > botPower * 1.25) &&
            !hasDroneAdvantage;
          ...
          const shouldAttack =
            ...
            (hasDroneAdvantage ||
              enemyWeak ||
              bot.drones >= enemyTarget.drones + 1 ||
              botPower >= enemyPower * 1.08);
      ...
      const safeCores = coresRef.current.map((core) => {
        ...
        // Daca zona verde ajunge peste un core, core-ul nu ramane in fum.
        // Dispare si apare instant random in interiorul cercului safe.
      ...
      if (
        ...
        // Programam urmatorul drop doar daca playerul a colectat toate core-urile.
        // Daca au fost mutate de zona verde, nu resetam drop-ul.
  ...
  }, [gameOver, gameMode, isMobileLandscape]);
  ...
  const applyMobileJoystickFromPointer = (e) => {
    ...
    // Sensibilitate ca in jocurile mobile:
    // dupa ~22% din raza joystick-ului, drona merge deja cu viteza maxima.
    // Astfel nu mai incetineste cand tii degetul mult in stanga/dreapta.
  ...
  return (
    <div
      className={`game-arena ${isMobileLike ? "is-mobile" : ""} ${
  ...
  {leaderboardPlayers.map((item, index) => (
    ...
    >
      ...
      <PixiArenaRenderer
        ...
        bots={gameMode === "ai" ? fullRenderBots : []}
        simpleBots={gameMode === "ai" ? simpleRenderBots : []}
        orbs={visibleOrbs}
      ...
      {showRotateOverlay && (
        <div className="mobile-landscape-lock">
          <div className="mobile-landscape-card">
            <div className="mobile-phone-icon">↻</div>
            <strong>Intoarce telefonul orizontal</strong>
            <span>Drone Swarm este optimizat pentru landscape. Jocul porneste cand ecranul este pe lat.</span>
          </div>
        </div>
      )}
