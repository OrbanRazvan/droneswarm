...
let GameGateway = class GameGateway {
    ...
    broadcastNormalRoomState(room, now) {
        ...
        const secondsUntilCoreDrop = room.cores.length === 0 && room.nextCoreWaveAt
            ...
            : null;
        const coreDropCountdown = secondsUntilCoreDrop && secondsUntilCoreDrop > 0 && secondsUntilCoreDrop <= Math.ceil(CORE_WARNING_DELAY / 1000)
            ? secondsUntilCoreDrop
            : null;
        const minimapOrbs = [...room.orbs]
            .sort((a, b) => a.id.localeCompare(b.id))
        ...
        for (const player of players) {
            ...
            const visiblePlayers = player.alive === false
                ? this.filterNear(viewAnchor, aliveOthers, VIEW_DISTANCE + 1200, NORMAL_VISIBLE_PLAYERS_LIMIT)
                    .map((other) => this.serializePlayer(other))
                : this.filterNear(player, players.filter((other) => other.id !== player.id), VIEW_DISTANCE, NORMAL_VISIBLE_PLAYERS_LIMIT).map((other) => this.serializePlayer(other));
    ...
    findClosestZonePvpItem(bot, items, zoneRadius, maxDistance, validator = null) {
        ...
        for (const item of items || []) {
            ...
            if (validator && !validator(item))
                continue;
            if (!this.isZonePvpPointSafeForBot(item.x, item.y, zoneRadius, 120))
                continue;
            ...
            let score = distance;
            if ((bot.hp || 0) <= ZONE_PVP_BOT_LOW_HP && item.type === 'nano')
                score -= 520;
            if ((bot.drones || 0) < 2 && item.type === 'swarm')
                score -= 420;
            if ((bot.drones || 0) >= 1 && (item.type === 'overclock' || item.type === 'berserk'))
                score -= 320;
            if ((bot.energy || 0) < 40 && !item.type)
                score -= 260;
            if (score < bestScore) {
                bestScore = score;
                best = item;
    ...
    findBestZonePvpCoreForBot(bot, room, zoneRadius) {
        return this.findClosestZonePvpItem(bot, room.cores, zoneRadius, ZONE_PVP_BOT_VIEW_RANGE, (core) => this.canUseCore(bot, core));
    }
    ...
    findZonePvpBotEnemy(bot, room) {
        ...
        for (const enemy of room.players.values()) {
            ...
            if (Math.abs(dx) > ZONE_PVP_BOT_VIEW_RANGE || Math.abs(dy) > ZONE_PVP_BOT_VIEW_RANGE)
                continue;
            const distance = Math.hypot(dx, dy);
            if (distance > ZONE_PVP_BOT_VIEW_RANGE)
                continue;
            ...
            if (!canWin)
                score += 320;
            if ((bot.hp || 0) <= ZONE_PVP_BOT_LOW_HP && enemyPower > botPower * 1.15)
                score += 560;
            if (distance < 260 && enemyPower > botPower * 1.15)
                score += 300;
            if (score < bestScore) {
                bestScore = score;
                bestEnemy = {
