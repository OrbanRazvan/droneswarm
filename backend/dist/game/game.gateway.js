...
export class GameGateway {
  ...
  private pushCombatEvent(
    ...
    // World snapshots are intentionally volatile to keep PvP smooth. Combat
    // feedback must not be volatile, otherwise a busy mobile connection can
    // miss the only packet containing -15 HP / SHIELD BLOCKED / kill rewards.
    // Send this tiny event directly and only to its owner; snapshots remain a
    // fallback and clients deduplicate by id.
    const eventName = room?.normalMode
      ? "normal-pvp:combat"
      : room?.zonePvpMode
        ? "zone-pvp:combat"
        : null;
    const viewerSocket = eventName
      ? this.server?.sockets?.sockets?.get(unit.id)
      : null;
    if (viewerSocket?.connected && eventName) {
      viewerSocket.emit(eventName, event);
    }
  }
