...
export class GameGateway {
  ...
  resolvePlayerPairCollision(a, b, room, now, zoneRadius) {
    ...
    // Zone PvP and online Battle Royale keep their contact-only rules, but
    // Normal PvP intentionally has physical drone bumps. The push is applied
    // only once per collision cooldown and uses the existing decaying impulse;
    // it does not add a per-client collision loop or expensive extra snapshots.
    ...
    const appliesPhysicalPush = !networkPvp || Boolean(room?.normalMode);
    if (appliesPhysicalPush) {
      const dirX = dx / dist;
      const dirY = dy / dist;
      const aImpulseX = -dirX * outcome.push;
      const aImpulseY = -dirY * outcome.push;
      const bImpulseX = dirX * outcome.push;
      const bImpulseY = dirY * outcome.push;
      ...
      if (room?.normalMode) {
        ...
        const baseEvent = {
          id: `normal-collision-${collisionVersion}-${key}`,
          serverTime: now,
          collisionVersion,
          // Current server transform is sent only as a fallback for a lost
          // snapshot; the visual uses the impulse, not a hard teleport.
          x: 0,
          y: 0,
        };

        this.emitNormalPvpCollisionImpulse(a, {
          ...baseEvent,
          x: a.x,
          y: a.y,
          impulseX: aImpulseX,
        ...
        this.emitNormalPvpCollisionImpulse(b, {
          ...baseEvent,
          x: b.x,
          y: b.y,
          impulseX: bImpulseX,
