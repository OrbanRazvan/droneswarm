...
export class GameGateway {
  ...
  /**
   * Reliable Zone PvP lobby snapshot.
   *
   * This is intentionally emitted on every join/rejoin and followed by an
   * immediate room broadcast. A player who is already waiting must see the
   * second player and the countdown without waiting for the next state tick.
   */
  private emitZonePvpJoined(client: Socket, room: any, player: any, now = Date.now()) {
  ...
  handleZonePvpJoin(
    ...
    // Leave other game modes, but do not remove this same Zone PvP player on
    // a reconnect / retry. Removing and re-adding him could transiently reset
    // a lobby countdown and split manual two-account tests.
    this.removePlayer(client.id);
    this.removeNormalPlayer(client.id);
    this.removeBattleRoyaleOnlinePlayer(client.id);

    const existingRoom = this.getZonePvpRoomBySocket(client.id);
    const existingPlayer = existingRoom?.players.get(client.id);
    if (existingRoom && existingPlayer && existingRoom.status !== "finished") {
      existingPlayer.userId = data?.isGuest ? null : data?.userId;
      existingPlayer.isGuest = Boolean(data?.isGuest);
      existingPlayer.username = String(
        data?.username || (data?.isGuest ? "Guest" : "Player"),
      ...
      existingPlayer.lastInputReceivedAt = now;
      this.markRoomOccupied(existingRoom);
      client.join(existingRoom.id);
      this.emitZonePvpJoined(client, existingRoom, existingPlayer, now);
      this.broadcastZonePvpRoomState(existingRoom, now);
      return;
    }
    ...
    this.emitZonePvpJoined(client, room, player, now);
    // This is the key lobby fix: notify the first account immediately that
    // the second account joined, instead of leaving its UI at 1 / 2.
  ...
  handleZonePvpInput(
    ...
    const now = Date.now();
    // Heartbeats arrive while the lobby/countdown overlay is shown too.
    // Refresh presence before the gameplay-only guard so the first account is
    // never removed after 30 seconds and the second account can always join it.
    player.lastSeenAt = now;
    player.lastInputReceivedAt = now;
  ...
  cleanupZonePvpRoom(room, now) {
    for (const player of room.players.values()) {
      ...
      // During waiting/countdown, a user may not press any key for minutes.
      // An open WebSocket is enough proof that the lobby member is present.
      // Only an active match uses the stale-input timeout.
      const shouldExpireActivePlayer =
        room.status === "playing" &&
        player.alive !== false &&
        now - Number(player.lastSeenAt || now) > 30000;
      if (shouldExpireActivePlayer) {
        this.removeZonePvpPlayer(player.id);
      }
    }
