...
// ---------------------------------------------------------------------------
// FIX PERFORMANTA: Battle Royale Online foloseste o zona (ZONE_START_RADIUS)
// mult mai mica decat harta intreaga (16000x16000), in timp ce Normal PvP
// foloseste NORMAL_ROOM_ZONE_RADIUS=100000 (mai mare decat harta, deci toate
// orburile incap oricum). Cu MAX_ORBS=1500 global aplicat si la BR Online,
// densitatea de orburi/energie pe ecranul VIZIBIL al jucatorului (toti
// jucatorii + toate orburile concentrate intr-un cerc mic, in loc de
// distribuite pe toata harta) devine mult mai mare decat la Normal PvP -
// mai multe forme de desenat per frame, randare WebGL mai costisitoare.
// Reducem densitatea tinta DOAR pentru room-urile battleRoyaleOnlineMode,
// fara sa afectam Normal PvP sau modul vechi pvp:.
// ---------------------------------------------------------------------------
const BR_ONLINE_MAX_ORBS = 350;
const BR_ONLINE_MAX_ENERGY_CELLS = 40;

// ---------------------------------------------------------------------------
// BATTLE ROYALE PLAYERS - mod nou, complet izolat de Battle Royale Online
// (battle-royale-online:*). Foloseste o harta proprie, mai mica (8000x8000 in
// loc de 16000x16000), special aleasa pentru pana la 50 jucatori reali fara
// sa aglomereze prea mult, dar suficient de mica ca densitatea de desen pe
// ecranul vizibil al unui jucator sa fie redusa pe device-uri slabe.
// Toate constantele/functiile pentru acest mod sunt prefixate cu BRP_ /
// BattleRoyalePlayers, ca sa nu existe nicio suprapunere accidentala cu
// vreun alt mod (pvp:, normal-pvp:, battle-royale-online:).
// ---------------------------------------------------------------------------
...
export class GameGateway {
    ...
    private battleRoyalePlayersRooms = new Map<string, any>();
    private battleRoyalePlayersSocketRoom = new Map<string, string>();
    ...
    @SubscribeMessage('battle-royale-players:join')
    handleBattleRoyalePlayersJoin(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
        this.removePlayer(client.id);
        this.removeNormalPlayer(client.id);
        this.removeBattleRoyaleOnlinePlayer(client.id);
        this.removeBattleRoyalePlayersPlayer(client.id);
    ...
    @SubscribeMessage('battle-royale-players:leave')
    handleBattleRoyalePlayersLeave(@ConnectedSocket() client: Socket) {
        this.removeBattleRoyalePlayersPlayer(client.id);
    }

    @SubscribeMessage('battle-royale-players:input')
    handleBattleRoyalePlayersInput(@ConnectedSocket() client: Socket, @MessageBody() input: any) {
    ...
    // =========================================================================
    // BATTLE ROYALE PLAYERS - functii complet izolate (prefix BattleRoyalePlayers /
    // helper-uri locale BRP*). Nu reutilizeaza functiile generice de mai sus
    // pentru ca acelea citesc WORLD_WIDTH/WORLD_HEIGHT module-level (16000x16000),
    // in timp ce acest mod ruleaza pe o harta proprie, mai mica (8000x8000).
    // =========================================================================
    ...
    getBattleRoyalePlayersZoneRadius(room) {
        if (!room.matchStartedAt) return BRP_ZONE_START_RADIUS;
        const elapsed = Math.max(0, Date.now() - room.matchStartedAt);
    ...
    updateBattleRoyalePlayersProjectiles(room, deltaFrames = 1) {
        ...
        for (const projectile of room.projectiles) {
            ...
            const traveled = Math.hypot(projectile.x - projectile.startX, projectile.y - projectile.startY);
            const age = Date.now() - (projectile.createdAt || Date.now());
            if (traveled > PROJECTILE_MAX_DISTANCE || age > PROJECTILE_MAX_LIFETIME) continue;
    ...
    applyBattleRoyalePlayersZoneDamage(room, now, zoneRadius) {
        ...
        for (const player of room.players.values()) {
            ...
            if (now - (player.lastZoneDamageAt || 0) < BRP_ZONE_DAMAGE_INTERVAL) continue;
    ...
    resolveBattleRoyalePlayersPairCollision(a, b, room, now, zoneRadius) {
        ...
        const dy = b.y - a.y;
        const dist = Math.hypot(dx, dy) || 1;
        if (dist > BRP_BODY_COLLISION_DISTANCE) return;
        ...
        if (now - lastAt < BRP_BODY_COLLISION_COOLDOWN) return;
        room.collisionCooldowns.set(key, now);
    ...
    collectBattleRoyalePlayersOrbs(room, zoneRadius) {
        ...
        for (const player of room.players.values()) {
            ...
            let collected = 0;
            for (let i = 0; i < room.orbs.length; i += 1) {
                if (collectedIndexes.has(i)) continue;
                const orb = room.orbs[i];
                const endDx = orb.x - player.x;
                const endDy = orb.y - player.y;
                const pathDistance = this.distancePointToSegment(
                ...
                if (
                    endDx * endDx + endDy * endDy > BRP_ORB_COLLECT_DISTANCE * BRP_ORB_COLLECT_DISTANCE &&
    ...
    collectBattleRoyalePlayersEnergy(room, zoneRadius) {
        ...
        for (const player of room.players.values()) {
            ...
            for (let i = 0; i < room.energyCells.length; i += 1) {
                ...
                if (
                    endDx * endDx + endDy * endDy > BRP_ENERGY_CELL_COLLECT_DISTANCE * BRP_ENERGY_CELL_COLLECT_DISTANCE &&
    ...
    collectBattleRoyalePlayersCores(room, zoneRadius) {
        ...
        for (const player of room.players.values()) {
            ...
            for (let i = 0; i < room.cores.length; i += 1) {
                ...
                if (
                    endDx * endDx + endDy * endDy > BRP_CORE_COLLECT_DISTANCE * BRP_CORE_COLLECT_DISTANCE &&
    ...
    broadcastBattleRoyalePlayersRoomState(room, now) {
        ...
        const coreDropCountdown =
            secondsUntilCoreDrop && secondsUntilCoreDrop > 0 && secondsUntilCoreDrop <= Math.ceil(BRP_CORE_WARNING_DELAY / 1000)
                ? secondsUntilCoreDrop
                : null;
